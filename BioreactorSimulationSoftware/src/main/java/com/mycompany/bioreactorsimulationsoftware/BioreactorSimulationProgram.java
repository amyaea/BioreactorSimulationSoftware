...
public class BioreactorSimulationProgram extends javax.swing.JFrame {
    ...
    public BioreactorSimulationProgram() {
        ...
        // Size of the program, 1261 is the width and 540 is the height
        this.setBounds(0, 0, 1261, 540);
        
        this.setTitle("Bioethanol Simulation");
        
        // Set the background color to dark grey later after loading the form
    ...
    private void initComponents() {
        ...
        safeRangesButton = new javax.swing.JButton();
        ...
        runButton.setBounds(10, 240, 163, 40);
        ...
        resetButton.setBounds(180, 240, 90, 40);
        getContentPane().add(jSeparator1);
        jSeparator1.setBounds(0, 290, 420, 10);
        ...
        jPanel1.setBounds(420, 0, 420, 500);
        ...
        jPanel2.setBounds(840, 0, 420, 500);

        safeRangesButton.setBackground(new java.awt.Color(255, 255, 153));
        safeRangesButton.setFont(new java.awt.Font("Liberation Sans", 0, 14)); // NOI18N
        safeRangesButton.setText("Safe ranges");
        safeRangesButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                safeRangesButtonActionPerformed(evt);
            }
        });
        getContentPane().add(safeRangesButton);
        safeRangesButton.setBounds(280, 240, 123, 40);
    ...
    private void resetButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_resetButtonActionPerformed
        ...
        // true here means the graph is empty
        createGraph(true);
    ...
    private void runButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_runButtonActionPerformed
        ...
        // A condition to check if the parameters are in the safe ranges
        ...
        // If it is within the safe ranges, continue with the rest of program, deriving the results and creating the graphs
        if(safeRange){

        // Constants declaration were here but now they are at line 31
        
        
        
        //Umax, which used to be a constant, is not a constant anymore
        ...
        // false means the graph is not empty, it is a parameter which is used to check, "is the graph empty?"
        createGraph(false);
        
        }
        
        // If they are not within the safe ranges, display an error message
        else JOptionPane.showMessageDialog(null, "The parameter values must be within the safe ranges, otherwise, they will yield unexcepted results!", "ERROR", ERROR_MESSAGE);
    }//GEN-LAST:event_runButtonActionPerformed

    private void safeRangesButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_safeRangesButtonActionPerformed
        // TODO add your handling code here:
        
        JOptionPane.showMessageDialog(null, "1g/L<=Initial biomass concentration<=44g/L\n0.6g=<Initial substrate amount<=200g\nFermentation time<100h\n0.5L<Broth Volume<50L\n100RPM<Impeller's speed<700RPM", "Safe range values", INFORMATION_MESSAGE);
    }//GEN-LAST:event_safeRangesButtonActionPerformed
 
    // emptyGraph is a boolean used to check if the graph is empty or not
    void createGraph(boolean emptyGraph){
        ...
        // Do the same for the second panel (Biomass & Ethanol graph)
        jPanel2.removeAll();
        jPanel2.repaint();
        ...
        // Since the graph is empty, we will set the data points 0
        s.add(0, 0);
        
        // Create a dataset using the series
        XYSeriesCollection ds = new XYSeriesCollection(s);
        
        // Creating the chart for the first panel (Substrate)
        ...
        // Create a ChartPanel (which is a Swing component to hold the chart) and add them to the JPanel, clear any previous components and refresh the panel
        jPanel1.add(cp1);
        jPanel1.revalidate();
        jPanel1.repaint();
        ...
        // If the graph is not empty and used to show the results
        if(!emptyGraph){
        ...
        // Creating the chart for the first panel
        JFreeChart chart1 = ChartFactory.createXYLineChart(
                "Substrate",                                   // Chart title
                "Time (h)",                                    // X Axis label
        ...
        chartPanel1.setPreferredSize(new java.awt.Dimension(510, 500));
        ...
        // We will use this timeSum to increment the time by 0.1 time unit
        double[] timeSum = {0};
        
        // Timer class to calcualte every 0.1 time unit of S, X(t) and P
        Timer timer = new Timer(100, e -> {
        
        // If the time got bigger than the fermentation time, STOP the timer!
        if (timeSum[0] > time) {
        ...
        // If not, we processed with the calcualtions for every 0.1 time unit
        ...
        // Draw the points on the graph after being done with calculations to make the graph live (like a video)
        SwingUtilities.invokeLater(() -> {
            ...
            //Debug
            System.out.println("Time unit="+timeSum[0]+"h");
        ...
        // Don't forget to increment the time by the desired time unit, which is here 0.1 in this case
        timeSum[0] += 0.1;
        
    });

    // Start the timer, this will be only executed once!
    timer.start();
    ...
    // Global test values
    static double iniBioConTest=25, iniSubConTest=150, timeTest=60, broVolTest=20, impSpeTest=400;
    
    public double returnUmax() {
            
            double UmaxTest = 0.03*(Math.pow(impSpeTest, 0.5));
            
            return UmaxTest;
            
        }
    
    public double returnXo() {
            
            double XoTest = iniBioConTest;
            
            return XoTest;
            
        }
    
    public double returnU() {
            
            double UTest = 0.6*(iniSubConTest/(6.026+iniSubConTest));
            
            return UTest;
            
        }
    
    public double returnX_t() {
            
            double X_tTest = 55.17/(1+((55.17-25)/25)*Math.exp(-0.5768269390998936*timeTest));
    ...
    public double returnP() {
            ...
            double PTest = 0.33 * 55.16999999999994 + 0.05 * timeTest;
            
            return PTest;
            
        }
    
    public double returnFinBio() {
            
            double finBioTest = 55.16999999999994;
            
            return finBioTest;
            
        }
    
    public double returnFinEthanCon() {
            
            double finEthanConTest = 21.20609999999998 / broVolTest;
            
            return finEthanConTest;
            
        }
    
    public double returnFinSubCon() {
            
            double subConsumedTest = 1.95652173913*1.0603049999999992*broVolTest;
        
            double finSubConTest = iniSubConTest - subConsumedTest;
            
            return finSubConTest;
            
        }
    
    public double returnUCost() { 
            
            double uCostTest = 0.65*1.0603049999999992;
    ...
    private javax.swing.JButton safeRangesButton;
